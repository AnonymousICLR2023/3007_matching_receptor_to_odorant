...
from Rec2Odorant.main.metrics import log_confusion_matrix, log_roc_curve, precision_recall_fscore, MCC, roc_auc, roc_curve, precision_recall_curve, average_precision_score, log_pr_curve # log_roc_curve_OLD, roc_auc_OLD
...
from Rec2Odorant.main.CLS_GNN_MHA.make_train_and_eval import make_train_epoch, make_valid_epoch, make_train_epoch_pmap, make_valid_epoch_pmap
from Rec2Odorant.main.CLS_GNN_MHA.make_regularization_loss import make_regularization_loss
...
def main_train(hparams):
    ...
    dataset = ProtBERTDatasetPrecomputeBERT(data_csv = os.path.join(datadir, hparams['SIZE_CUT_DIRNAME'], hparams['TRAIN_CSV_NAME']),
                        class_weight_json = None, # NOTE: Legacy, not used 
    ...
    if hparams['LOADER_OUTPUT_TYPE'] == 'jax':
        ...
        loader = _loader.tf_Dataset(output_signature = get_tf_specs(is_weighted = hparams['WEIGHT_COL'] is not None,
                                                                    n_partitions = hparams['N_PARTITIONS'], 
        ...
        valid_loader = _valid_loader.tf_Dataset(output_signature = get_tf_specs(is_weighted = hparams['VALID_WEIGHT_COL'] is not None, 
                                                                                n_partitions = hparams['N_PARTITIONS'], 
                                                                                num_node_features = len(hparams['ATOM_FEATURES']), 
                                                                                num_edge_features = len(hparams['BOND_FEATURES']),
                                                                                padding_n_node = hparams['PADDING_N_NODE'],
        ...
        if hparams['SIZE_CUT_DIRNAME'] is not None:
            big_loader = _big_loader.tf_Dataset(output_signature = get_tf_specs(is_weighted = hparams['WEIGHT_COL'] is not None,
                                                                            n_partitions = hparams['N_PARTITIONS'], 
                                                                            num_node_features = len(hparams['ATOM_FEATURES']),
                                                                            num_edge_features = len(hparams['BOND_FEATURES']),
                                                                            padding_n_node = hparams['BIG_PADDING_N_NODE'],
            ...
            big_valid_loader = _big_valid_loader.tf_Dataset(output_signature = get_tf_specs(is_weighted = hparams['VALID_WEIGHT_COL'] is not None,
                                                                                    n_partitions = hparams['N_PARTITIONS'], 
                                                                                    num_node_features = len(hparams['ATOM_FEATURES']), 
                                                                                    num_edge_features = len(hparams['BOND_FEATURES']),
                                                                                    padding_n_node = hparams['BIG_PADDING_N_NODE'],
    ...
    while epoch <= hparams['N_EPOCH']:
        ...
        # with SummaryWriter(os.path.join(logdir, 'validation')) as valid_writer:
        if True:
            summary = Summary()
            roc_curve_values = roc_curve(CM_per_threshold, drop_intermediate=True)
            pr_curve_values = precision_recall_curve(CM_per_threshold)
