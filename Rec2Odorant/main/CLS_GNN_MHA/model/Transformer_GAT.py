...
from Rec2Odorant.main.CLS_GNN_MHA.layers.MPattns import PositionwiseFeedForwardNetwork
...
class _multiHead_GAT(flax.linen.Module):
    ...
    @staticmethod
    def make_node_update_fn(LN_in, FFN, LN_out):
    ...
    def __call__(self, G):
        ...
        for i in range(self.num_heads):
            _G = jraph.GAT(attention_query_fn = self.make_attention_query_fn(flax.linen.Dense(features = self.node_d_model // self.num_heads, use_bias = True)),
                        ...
                        node_update_fn = self.make_node_update_fn(LN_in = flax.linen.LayerNorm(), 
                                                            FFN = PositionwiseFeedForwardNetwork(self.widening_factor), 
                                                            LN_out = flax.linen.LayerNorm()))(G)
...
class Transformer_GAT_model(flax.linen.Module):
    ...
    bond_features : Sequence = () # ('BondType', 'Stereo', 'IsAromatic') # Bond features must be empty
    # Internal attributes:
    _eps = 10e-7
    atom_embed_funcs = {}
    ...
    def setup(self):
        print('\n----\nWARNING: This is only attention layers. Maybe some FFN and LN would be needed.')
        # Atom embedding:
