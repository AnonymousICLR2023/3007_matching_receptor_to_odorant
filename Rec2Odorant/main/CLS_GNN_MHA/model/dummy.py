...
from Rec2Odorant.main.CLS_GNN_MHA.layers.attention import MultiHeadDotProductAttention_with_AttnWeights
from Rec2Odorant.main.CLS_GNN_MHA.layers.gnn import ECCLayer, GlobalAttnSumPoolLayer
from Rec2Odorant.main.CLS_GNN_MHA.layers.MPattns import GraphProcessingEncoderLayer

from Rec2Odorant.main.CLS_GNN_MHA.model.essentials.EdgeGatedMPNN import GatedTruncatedNormalDynamicMessagePassing
...
class DummyModel(flax.linen.Module):
    ...
    def setup(self):
        ...
        # ----------- 0:
        self.mha_mpnn_Q = GatedTruncatedNormalDynamicMessagePassing(edge_embedding_size=self.edge_embedding_size, message_activation=self.mpnn_message_activation, a=3, b=9, mean=6.0, stddev=1.0)
        self.mha_mpnn_K = GatedTruncatedNormalDynamicMessagePassing(edge_embedding_size=self.edge_embedding_size, message_activation=self.mpnn_message_activation, a=3, b=9, mean=6.0, stddev=1.0)
        self.mha_mpnn_V = GatedTruncatedNormalDynamicMessagePassing(edge_embedding_size=self.edge_embedding_size, message_activation=self.mpnn_message_activation, a=3, b=9, mean=6.0, stddev=1.0)
