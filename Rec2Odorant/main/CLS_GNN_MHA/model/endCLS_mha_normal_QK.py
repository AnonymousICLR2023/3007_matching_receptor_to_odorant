...
from Rec2Odorant.main.CLS_GNN_MHA.layers.MPattns import GraphProcessingEncoderLayer, PositionwiseFeedForwardNetwork
...
class endCLS_mha_normal_QK_model(flax.linen.Module):
    ...
    def setup(self):
        ...
        self.dense_1 = flax.linen.Dense(256)
        self.out_mha = MultiHeadDotProductAttention_with_AttnWeights(num_heads = 4, dropout_rate = 0.3)
        self.out_LN_1 = flax.linen.LayerNorm()
        self.out_FFN = PositionwiseFeedForwardNetwork(widening_factor = 8)
        self.out_LN_2 = flax.linen.LayerNorm()
    ...
    def __call__(self, inputs, deterministic):
        ...
        S = self.dense_1(S)
        S = jnp.reshape(S, (batch_size, 1, -1))
        x = jnp.reshape(mols.nodes, (batch_size, -1, self.node_d_model))
        mask = jnp.reshape(mols_padding_mask, (batch_size, 1, 1, -1))
        x = self.out_mha(S, x, mask = mask, deterministic = deterministic)
