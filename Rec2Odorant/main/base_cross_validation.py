...
class BaseCrossValidation:
    """
    base class for CrossValidation data generations.
    """
    ...
    def _get_working_dir(self):
        """
        Create direcotry as a subdirectory in base_working_dir. The direcotry is created in 
        <self.base_working_dir>/<self.func_data_name>/<self.func_split_data_name>/<_datetime>
        where _datetime is current datetime in "%Y%m%d-%H%M%S" format.

        This function is called whenever CV_data is called.
        """
        _datetime = datetime.datetime.now().strftime("%Y%m%d-%H%M%S")
        self.working_dir = os.path.join(self.base_working_dir, self.func_data_name, self.func_split_data_name, _datetime)
        if not os.path.exists(self.working_dir):
            os.makedirs(self.working_dir)
    ...
    def _serialize_attributes(self):
        if not self.read_data_name or not self.func_data_name or not self.func_split_data_name:
            raise ValueError('functions must have names. Check if all have read_data_name or func_data_name or func_split_data_name set.')
        hparams = { 'working_dir' : self.working_dir,
                    'data_path' : self.data_path,
                    'read_data' : self.read_data_name,
    ...
    def save_hparams(self):
        hparams = self._serialize_attributes()
        with open(os.path.join(self.working_dir, 'CV_hparams.json'), 'w') as outfile:
            json.dump(hparams, outfile)
    ...
    def CV_data(self):
        ...
        if data_train is not None:
            print("Shape of data_train:    {}".format(data_train.shape))
            data_train.to_csv(os.path.join(self.working_dir, 'data_train.csv'), sep = ';', index = True, header = True)

        # Save validation data:
        if data_valid is not None:
            print("Shape of data_valid:    {}".format(data_valid.shape))
            data_valid.to_csv(os.path.join(self.working_dir, 'data_valid.csv'), sep=';', index = True, header = True)

        # Save test data:
        if data_test is not None:
            print("Shape of data_test:    {}".format(data_test.shape))
            data_test.to_csv(os.path.join(self.working_dir, 'data_test.csv'), sep=';', index = True, header = True)
