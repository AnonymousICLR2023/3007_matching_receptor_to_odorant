...
class CV_ORpairs(BaseCVPreProcess):
    ...
    def func_data(self, raw_data):
        ...
        # data.set_index(['mol_id', 'seq_id'], inplace = True)
...
if __name__ == '__main__':
    if False:
        df = pandas.read_csv('/home/matej/Documents/repos/MatejHl/ChemosimDB/ChemosimDB/Data/Exports/export_20220324-192913.csv', sep = ';', index_col = 0)
        ...
        print(df['Unit_Screen'].unique())
        print(df['Responsive'].unique())
        print(df['nbr_measurements'].unique())
        print(df['Type'].unique())
        print(df['Cell_line'].unique())
        print(df['Co_transfection'].unique())
        print(df['Assay System'].unique())
        print(df['Tag'].unique())
        print(df['Mixture'].unique())    

        print(df[(df['Parameter'] == 'ec50')&(df['Responsive'] == 0)]['Value'].unique())

        print(df.groupby('Responsive').count()['DOI'])
        print(df[df['Parameter'] == 'ec50'].groupby('Responsive').count()['DOI'])

        print(df[(df['Parameter'] == 'ec50') & (df['Value'] == '0')][['Responsive', 'Value','Parameter', 'Reference']])
        print('------------')
        ...
        # bla = df['canonicalSMILES']
        # bla = bla[~bla.isna()]
        # print(bla)
        # print(enumerate_isomers('CC(C)CCO'))


    print('-------------')
    import time
    # os.environ['CUDA_VISIBLE_DEVICES'] = '1'
    # test = CV_ORpairs_DiscardMix(base_working_dir = os.path.join('datasets', 'chemosimdb_confidence', 'tmp'), # os.path.join('BERT_GNN', 'Data', 'chemosimdb')
    #                                     data_path = {'pairs': '/home/matej/Documents/repos/MatejHl/ChemosimDB/ChemosimDB/Data/Exports/export_20220512-135815.csv',
    #                                                 'uniprot_sequences': '/home/matej/Documents/repos/MatejHl/ChemosimDB/ChemosimDB/Data/uniprot_sequences.csv',
    ...
    test = CV_ORpairs_OnlyMix(base_working_dir = os.path.join('datasets', 'chemosimdb_confidence', 'tmp'), # os.path.join('BERT_GNN', 'Data', 'chemosimdb')
                                        data_path = {'pairs': '/home/matej/Documents/repos/MatejHl/ChemosimDB/ChemosimDB/Data/Exports/export_20220512-135815.csv',
                                                    'uniprot_sequences': '/home/matej/Documents/repos/MatejHl/ChemosimDB/ChemosimDB/Data/uniprot_sequences.csv',
                                                    'map_inchikey_to_isomericSMILES' : None, # '/home/matej/Documents/repos/MatejHl/Receptor_odorant/Receptor_odorant/JAX/datasets/chemosimdb_confidence/tmp/all_mixEnsamble/map_inchikey_to_isomericSMILES.json',
                                                    'map_inchikey_to_canonicalSMILES' : None, # '/home/matej/Documents/repos/MatejHl/Receptor_odorant/Receptor_odorant/JAX/datasets/chemosimdb_confidence/tmp/all_mixEnsamble/map_inchikey_to_canonicalSMILES.json',
