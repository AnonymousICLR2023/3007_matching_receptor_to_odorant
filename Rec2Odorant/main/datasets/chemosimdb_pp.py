...
if __name__ == '__main__':
    ...
    # weighted_Harmonic = CVPP_weighted_Harmonic(data_dir = os.path.join('BERT_GNN','Data','chemosimdb','all_mixEnsamble','EC50_random_data','20220325-121205'))
    # weighted_Harmonic.postprocess()
    # weighted_LogHarmonic = CVPP_weighted_LogHarmonic(data_dir = os.path.join('BERT_GNN','Data','chemosimdb','all_mixEnsamble','EC50_random_data','20220325-121205'))
    # weighted_LogHarmonic.postprocess()
    # class_dist = CVPP_class_dist(data_dir = weighted_LogHarmonic.working_dir)
    # class_dist.postprocess()

    # sizecut = CVPP_SizeCut(data_dir =  weighted_LogHarmonic.working_dir)
    # sizecut.postprocess()


    # data_dir = os.path.join('datasets', 'chemosimdb_confidence', 'tmp', 'all_mixEnsamble_20220512-152626', 'EC50_random_data', '20220512-153300') # 20220512-144631

    # weighted_Harmonic = CVPP_weighted_Harmonic(data_dir = data_dir)
    # weighted_Harmonic.postprocess()
    # weighted_LogHarmonic = CVPP_weighted_LogHarmonic(data_dir = data_dir)
    # weighted_LogHarmonic.postprocess()
    # class_dist = CVPP_class_dist(data_dir = weighted_LogHarmonic.working_dir)
    # class_dist.postprocess()

    # combine_weight_columns = CombineWeightsCVPP_LogHarmonic_Class(data_dir = data_dir, 
    #                                                     auxiliary_data_path = {'class_dist' : os.path.join(data_dir, 'class_dist.json'),
    #                                                                 'screening_confidence_probs' : os.path.join('datasets','chemosimdb_confidence','Screening_confidence_probabilities.json'),
    #                                                                 })
    # combine_weight_columns.postprocess()

    # concatGraph = CVPP_Mixture_ConcatGraph(data_dir = data_dir, 
    #                                                     auxiliary_data_path = {'map_inchikey_to_isomericSMILES' : None,
    #                                                                             'map_inchikey_to_canonicalSMILES' : None})
    # concatGraph.postprocess()

    # sizecut = CVPP_SizeCut(data_dir =  combine_weight_columns.working_dir)
    # sizecut.postprocess()
    
    # datasets/chemosimdb_confidence/tmp/all_mixEnsamble/EC50_random_data/20220511-160104/data_test.csv
    
    # data_dir = os.path.join('datasets', 'chemosimdb_confidence', 'tmp', 'mixDiscard_20220519-121655', 'EC50_LeaveOut_OR', '20220520-105928')
    data_dir = os.path.join('datasets', 'chemosimdb_confidence', 'tmp', 'mixDiscard_20220519-121655', 'EC50_LeaveOut_Mol', '20220520-141601')
    
    # test = CVPP_DiscardTest_OR_PosNeg(data_dir = data_dir)
    # test.postprocess()

    # test = CVPP_DiscardTest_OR_KeepNeg(data_dir = data_dir)
    # test.postprocess()
    
    # test = CVPP_DiscardTest_Mol_PosNeg(data_dir = data_dir)
    # test.postprocess()

    test = CVPP_DiscardOrphanOR(data_dir = data_dir)
    test.postprocess()
