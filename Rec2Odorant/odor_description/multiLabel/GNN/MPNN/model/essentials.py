...
# from Custom_Flax_code.normalization.graphNorm import GraphNorm
...
class MPNN_update_node(flax.linen.Module):
    ...
    def __call__(self, inputs):
        ...
        #print(x.shape, received_messages.shape)
        x, _ = flax.linen.GRUCell()(carry = x, inputs = sent_messages) # activation_fn = self.message_activation
...
class TNDMPNNStep(flax.linen.Module):
    ...
    def setup(self):
        ...
        # self.GroupNorm_nodes = flax.linen.GroupNorm(num_groups=1)
        # self.GroupNorm_edges = flax.linen.GroupNorm(num_groups=1)
        # self.norm = GraphNorm()  ### Maybe use InstanceNorm 
    ...
    def __call__(self, graph, dummy):
        ...
        G = graph
        G = self.gn(G)
        # G = G._replace(nodes = self.GroupNorm_nodes(G.nodes))
        # G = G._replace(edges = self.GroupNorm_edges(G.edges))
        return G, G
...
class MPNN(flax.linen.Module):
    ...
    def __call__(self, inputs):
        ...
        """
        G = inputs
        mpnn = TNDMPNNStep(self.edge_embedding_size, self.message_activation, name = 'TNDMPNNStep')
