...
from Rec2Odorant.odor_description.utils import deserialize_to_jraph, create_line_graph_and_pad
from Rec2Odorant.odor_description.base_loader import BaseDataset, BaseDataLoader
...
class Collate:
    ...
    """
    def __init__(self, mid_padding_n_node, mid_padding_n_edge, max_padding_n_node, max_padding_n_edge, n_partitions = 0):
        self.mid_padding_n_node = mid_padding_n_node
        self.mid_padding_n_edge = mid_padding_n_edge
        self.max_padding_n_node = max_padding_n_node
        self.max_padding_n_edge = max_padding_n_edge 
    ...
    def _graph_collate(self, batch):
        ...
        """
        mid_padding_n_node = self.mid_padding_n_node
        mid_padding_n_edge = self.mid_padding_n_edge
        max_padding_n_node = self.max_padding_n_node
        max_padding_n_edge = self.max_padding_n_edge
        n_partitions = self.n_partitions

        max_n_node = max([sum(m.n_node) for m in batch])
        max_n_edge = max([sum(m.n_edge) for m in batch])
        mols = []
        if max_n_node < mid_padding_n_node and max_n_edge < mid_padding_n_edge:
            padding_n_node = mid_padding_n_node
            padding_n_edge = mid_padding_n_edge
        else:
            padding_n_node = max_padding_n_node
            padding_n_edge = max_padding_n_edge
        for mol in batch:
...
class DatasetBuilder(BaseDataset):
    ...
    def read(self):
        ...
        if self.weight_col is not None:
            df['_weight'] = df[self.weight_col].apply(ast.literal_eval)
        if self.label_col is not None:
            df['_label'] = df[self.label_col].apply(ast.literal_eval)
        return df
